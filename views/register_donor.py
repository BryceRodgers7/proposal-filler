"""
Registration page for donors with email verification.
"""
...
def render_register_donor_page():
    ...
    with st.form("register_donor_form"):
        ...
        with col1:
            first_name = st.text_input("First Name *", key="donor_first_name")
        with col2:
            last_name = st.text_input("Last Name *", key="donor_last_name")
        
        company = st.text_input("Company / Organization", key="donor_company", help="Optional")
        
        st.markdown("---")
        st.markdown("#### Address Information")
        
        street_address = st.text_input("Street Address *", key="donor_street_address")
        
        col_city, col_state, col_zip = st.columns([2, 1, 1])
        with col_city:
            city = st.text_input("City *", key="donor_city")
        with col_state:
            state = st.text_input("State *", key="donor_state")
        with col_zip:
            zip_code = st.text_input("ZIP *", key="donor_zip")
        
        phone_number = st.text_input("Phone Number", key="donor_phone", help="Optional")
...
def _render_registration_success():
    ...
    ">
        ...
        <ol style="color: #15803d; margin: 0; padding-left: 20px;">
            ...
            <li>Return here to log in and start browsing organizations!</li>
        </ol>
    </div>
    """, unsafe_allow_html=True)
